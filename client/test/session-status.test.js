// 세션 상태 해석기 단위 테스트 — 화면에서 상태 텍스트가 사라졌으므로(이제는 이름
// 오른쪽 미니 마스코트의 포즈뿐) 라벨·상태 클래스 매핑이 틀리면 사용자에게 알릴 방법이
// 전혀 없다. e2e에는 질문 대기(AskUserQuestion) 시나리오 픽스처가 없어 그 분기는
// 여기서만 고정된다.
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { statusDotOf } from '../src/lib/session-status.js';

test('statusDotOf: 다섯 상태가 각각 다른 상태 클래스를 받는다', () => {
...
test('statusDotOf: 질문 대기는 라벨만 갈리고 클래스는 권한 대기와 공유한다', () => {
  ...
  assert.equal(question.cls, perm.cls, '같은 "확인 필요" 상태라 클래스는 하나(마스코트도 같은 포즈)');
...
test('statusDotOf: 모르는 상태는 상태 클래스 없이 원문 라벨로 폴백한다', () => {
