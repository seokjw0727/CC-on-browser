...
- **원격 제어(Remote Control)** — 입력창 위 `📱 원격 제어` pill로 켜면, 지금 열어 둔
  레포를 **claude.ai/code와 Claude 모바일 앱에서 조종**할 수 있습니다. 팝오버에 접속
  링크·주소 복사·끄기가 있습니다. **사이드바 세션 행을 우클릭**(키보드는 Shift+F10)해도
  켜고 끌 수 있어, 지금 보고 있지 않은 다른 프로젝트의 세션도 다룰 수 있습니다.
  켜져 있는 동안에는 **브라우저를 닫아도 앱이 종료되지
  않습니다**(폰에서 계속 쓰라고 켠 기능이므로) — 다 쓰면 꺼 주세요. 노출 범위는
  [SECURITY.md](SECURITY.md)를 읽어 보세요.
...
8. **원격 제어(선택)** — 입력창 위 `📱 원격 제어`를 누르거나 사이드바 세션 행을
   우클릭하면 이 레포를 claude.ai/code·모바일 앱에서 조종할 수 있습니다. 팝오버의
   링크로 접속하고, 다 쓰면 **끄기**를 누르세요 — 켜 둔 동안에는 브라우저를 닫아도
   앱이 살아 있습니다.
9. **실행 중 작업 보기** — 입력창 아래 도크에서 지금 돌고 있는 셸·서브에이전트를 확인하고,
   누르면 대화의 해당 위치로 이동합니다.
10. **종료** — 탭을 모두 닫으면 약 10초 뒤 자동 종료(`--no-open`은 `Ctrl+C`). 절전·리드
   닫힘은 종료로 치지 않습니다. **원격 제어가 켜져 있으면 종료하지 않습니다.**
