...
- **원격 제어(Remote Control)** — 입력창 위 `📱 원격 제어` pill로 켜면, 지금 열어 둔
  ...
  링크·주소 복사·끄기가 있습니다. 켜져 있는 동안에는 **브라우저를 닫아도 앱이 종료되지
...
8. **원격 제어(선택)** — 입력창 위 `📱 원격 제어`를 누르면 이 레포를 claude.ai/code·
   모바일 앱에서 조종할 수 있습니다. 팝오버의 링크로 접속하고, 다 쓰면 **끄기**를
   누르세요 — 켜 둔 동안에는 브라우저를 닫아도 앱이 살아 있습니다.
